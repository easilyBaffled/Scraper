...
var socket = (0, _socket2.default)('https://rwm.herokuapp.com/', { jsonp: false });

var Scraper = function (_Component) {
  _inherits(Scraper, _Component);
  ...
  function Scraper(props) {
    ...
    _this._resetSelection = function () {
      _this.setState({ selectorMode: true, confirmation: false, targetPath: '' });
      document.onmousemove = function (event) {
        var target = event.target;
        var targetStyle = target.style;
        target.style.outline = '2px solid gold';
        target.onclick = function (clickEvent) {
          clickEvent.preventDefault();
          clickEvent.stopPropagation();
          document.onmousemove = '';
          _this.setState({ selectorMode: false, confirmation: true, targetPath: _this._cssPath(clickEvent.target) });
        };
    ...
    document.onkeydown = function (keyEvent) {
      if (keyEvent.ctrlKey && keyEvent.keyCode === 65 && _this.state.selectorMode) {
        _this.setState({ selectorMode: false });
        location.reload();
      } else if (keyEvent.ctrlKey && keyEvent.keyCode === 65 && !_this.state.selectorMode) {
        _this.setState({ selectorMode: true });
  ...
  _createClass(Scraper, [{
    ...
    value: function render() {
      ...
      return _react2.default.createElement(
        'div',
        { id: 'scraper', style: { display: 'flex', flexDirection: 'column', justifyContent: 'center', alignItems: 'center' } },
        this.state.confirmation ? _react2.default.createElement(
          'div',
          { style: { position: 'absolute', height: '100%', width: '100%', top: '0', left: '0', color: 'white', backgroundColor: 'rgba(100, 100, 100, .8)', zIndex: '9999', display: 'inlineBlock' } },
          ...
          _react2.default.createElement(
            'h3',
            null,
            document.querySelector(this.state.targetPath).innerText
          ),
          _react2.default.createElement(
            'button',
            { onClick: this._acceptSelection },
            'OK'
          ),
          _react2.default.createElement(
            'button',
